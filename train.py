...
class CombinedLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        if predictions.size(1) == 1:  # Binary segmentation
            ...
            dice_loss = 1 - (2 * torch.sum(predictions_sigmoid * targets) + self.smooth) / \
            ...
            for c in range(softmax_pred.shape[1]):
                ...
                dice_loss_total += 1 - (2 * intersection + self.smooth) / (union + self.smooth)
...
def calculate_dice_score(predictions, targets):
    ...
    dice = (2 * intersect + smooth) / (torch.sum(predictions) + torch.sum(targets) + smooth)
    return dice.item()
...
def main():
    ...
    try:
        ...
        print(f"Data loaded - Train: {len(train_loader)} batches, Val: {len(val_loader)} batches")
    ...
    print(f"Model initialized - Total params: {total_params:,}, Trainable params: {trainable_params:,}")
    ...
    for epoch in range(config.NUM_EPOCHS):
        ...
        # Print epoch results
        print(f"Train Loss: {train_loss:.4f} | Train Dice: {train_dice:.4f} | Train CE: {train_ce:.4f}")
        print(f"Val Loss: {val_loss:.4f} | Val Dice: {val_dice:.4f} | LR: {current_lr:.2e}")
        ...
        if val_dice > best_val_dice:
            ...
            create_checkpoint(model, optimizer, scheduler, epoch+1, best_val_dice, best_model_path)
        ...
        if (epoch + 1) % 10 == 0:
            periodic_model_path = os.path.join(config.CHECKPOINT_DIR, f"model_epoch_{epoch+1}.pth")
            create_checkpoint(model, optimizer, scheduler, epoch+1, val_dice, periodic_model_path)
